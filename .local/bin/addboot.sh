...
efibootmgr -d $device -p 1 -c -L "Arch Linux" -l /vmlinuz-linux -u "rd.luks.name=$device_uuid=$cryptsetup_name resume=UUID=$swap_uuid root=UUID=$root_uuid initrd=\initramfs-linux.img initcall_blacklist=sysfb_init iommu=pt rw"
